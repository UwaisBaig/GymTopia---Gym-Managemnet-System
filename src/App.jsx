        ...
        <div style={{ maxWidth:1250, margin:"0 auto", padding:"0 16px", display:"flex", alignItems:"center", height:58 }}>
          ...
          <div style={{ display:"flex", gap:2, flex:1, overflowX:"auto" }}>
            {tabs.map(t=>(
              ...
              <button key={t.id} onClick={()=>onTab(t.id)} style={{ padding:"6px 14px", background:"none", border:"none", cursor:"pointer", fontSize:12, fontWeight:700,
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginTop:4 }}>
            {PACKAGES.map(p=>(
              <div key={p.label} onClick={()=>setF(x=>({...x,pkg:p.label}))}
                style={{ border:`${f.pkg===p.label?`2px solid ${RED}`:`1px solid ${BORDER}`}`, borderRadius:8, padding:"12px 10px", cursor:"pointer", textAlign:"center",
...
function RevenueView({ members }) {
  ...
  const byPkg   = PACKAGES.map(p=>({ label:p.label, count:members.filter(m=>m.getPackage()===p.label).length,
...
export default function App() {
  ...
  const toggleFee = useCallback(id=>{
    setMembers(prev=>{
      const n=prev.map(m=>{ if(m.getId()!==id) return m;
